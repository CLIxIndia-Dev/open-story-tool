...
//        console.log(audioButtonDiv)
//        audioButtonDiv.style.display = 'none'
//        videoButtonDiv.style.display = 'none';
//        navigator.mediaDevices.enumerateDevices()
//            .then(function(devices) {
//                devices.forEach(function(device) {
//                    console.log(device.kind);
//                    if (device.kind == "videoinput") {
//                        videoButtonDiv.style.display = '';
//                    }
//                    if (device.kind == "audioinput") {
//                        audioButtonDiv.style.display = '';
//                    }
//                });
//            })
//        .catch(function(err) {
    ...
    var Data = (function () {
        ...
        Data.prototype.startSession = function () {
            try {
                ...
                if (this.sesssion.length === 0) { // probably wont ever be needed but this is the legacy way
...
(function (Slideshow) {
    ...
    (function (State) {
        ...
        //        State[State["Projects"] = 8] = "Projects";
    })(State || (State = {}));
    ...
    var SlideshowApp = (function () {
        function SlideshowApp(element) {
            ...
            this.api = (this.URLparams.get("api") || "/api/v1/logging/genericlog");
            this.data.remoteLocation = this.api;
            // this.data.remoteLocation = "/api/appdata/"; // from unplatform
            ...
            if (this.imgFolder == undefined) { // default for when no gallery specified
                this.imgFolder = "";
                img_path = "images/files.json";
                //alert('inside undefined');
            }
            else { // location of gallery otherwise
                startParams["gallery"] = this.imgFolder;
                this.imgFolder = this.imgFolder + "/";
                img_path = "images/" + img_Folder +"/files.json";
            }
            var jqXHR = $.getJSON(img_path, function (json) {
                _this.loadGallery(json);
            }).fail(function () {
                $.getJSON(img_path, function (json) {
                    _this.loadGallery(json);
                    //alert(img_path);
                });
            });
            ...
            $(this.ui.useWebcamBtn).on("click", function () {
                if (!_this.videoEnabled && (!_this.webcam || !_this.webcam.streaming)) {
            ...
            $(this.ui.projectsLoadBtn).on("click", function () {
                var remoteProjects = "Please select a project to import:<br>";
                var options = projectManager.projectOptions;
                remoteProjects += "<select id='projectDropdown'>" + options + "</select>";
                _this.ui.showDialog(remoteProjects, [
                    {
                        text: "OK",
                        fn: function () {
                            // somethingfrom radio or dropdown button?
                            var projDropdown = document.getElementById("projectDropdown");
                            var projSelection = projDropdown.options[projDropdown.selectedIndex].value;
                            //                            document.projectManager.loadProject(projSelection)
                            projectManager.loadProject(projSelection);
                        }
                    },
                    {
                        text: "Cancel",
            ...
            $(this.ui.projectsSaveBtn).on("click", function () {
                _this.exportMode = 3; // remote
                _this.saveSlides();
            });
        ...
        SlideshowApp.prototype.initMedia = function (callback, av = {video: true, audio: true}) {
            var _this = this;
            navigator.getMedia = (navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia);
            navigator.getMedia(av, function (stream) {
                if (av.video) {
                ...
                console.log("Webcam or audio not accessible: " + error.message);
            });
        };
        ...
        SlideshowApp.prototype.finishSave = function () {
            ...
            var zip = new JSZip();
            // https://stuk.github.io/jszip/documentation/api_jszip/generate_async.html#mimetype-option
            zip.file("mimetype", "application/zip/ost");
            ...
            if (this.exportMode != 3) {
                ...
                ext = "cssw";
                projectManager.saveProject(content, filename);
...
(function (Slideshow) {
    var UI = (function () {
        function UI(content) {
            ...
            this.projectsLoadBtn = document.getElementById("projectsLoadBtn");
            this.projectsSaveBtn = document.getElementById("projectsSaveBtn");
...
class ProjectManager {
    constructor() {
        //        is workspace required?
        //        "api/v1?resource_type=OpenStoryTool&workspace=desk&created_by=" 

        //        this.url = "/api/v1?resource_type=OpenStoryTool"
        this.url = "/api/v1";
        this.user_id = this.getCookie("user_id");
        if (this.user_id == "None") {
            this.user_id = 2; // api requires a user to not be none
        }
    ...
    getCookie (name) { return Slideshow.Utils.getCookie(name); }
    getProjects () {
        ...
        var xhr = new XMLHttpRequest();
        console.log(this.user_id);
        xhr.open("GET", this.url + "?if_file.mime_type=application/zip&created_by=" + this.user_id, true);
        xhr.onload = function (e) {
            ...
            if (xhr.readyState === 4) {
                ...
                if (xhr.status === 200) {
                    ...
                    _this.projectsAvailable = true;
                    console.log(xhr.responseText);
                    _this.projectResponse = JSON.parse(xhr.responseText);
                    _this.listProjects();
                } else {
    ...
    listProjects () {
        ...
        this.projectMetadata = {};
        // repopulate the menu
        for (var i in this.projectResponse) {
            // generate options & metadata
            var proj = this.projectResponse[i];
            console.log(proj);
            var projName;
            if (proj.name != null) {
                projName = proj.name.toString();
            } else {
                proj = "no name";
            }
            
            var projUrl = proj.if_file.original.relurl.toString();
            
            this.projectOptions += "<option value=" + projUrl + ">" + projName + "</option>";
            
            this.projectMetadata[projUrl] = projName;
        }
    ...
    loadProject(projUrl){
        ...
        var projName = this.projectMetadata[projUrl];
        console.log(this.projectMetadata);
        console.log(projUrl);
        // Give it a default extension if it doesn't have one
        var projExt = projName.split(".").pop().toString().toLowerCase();
        if (!["cssw", "csst", "cssv"].includes(projExt)) {
            projName += ".cssw"; // default
        }   
        
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "/media/" + projUrl, true);
        xhr.responseType = "blob";
        xhr.onload = function (e) {
            if (xhr.readyState === 4) {
                console.log("readystate");
    ...
    saveProject(file, name){
        //        slideApp.exportMode = 3
        console.log(file);
        console.log(name);
        
        var csrf = projectManager.getCookie("csrftoken");

        
        var formData = new FormData();
        formData.append("filehive", file, name);
        //        formData.append("content", "sample content");
        //        formData.append("name", name);
        //        this.formData.append("tags", "check, 1, 2");
        formData.append("user_name", "OST user");
        //        formData.append("workspace", "desk");
        
    
        formData.append("created_by", projectManager.user_id);
        //        formData.append('csrftoken', csrf)                
        
        var xhr = new XMLHttpRequest();
        
        xhr.open("POST", projectManager.url + "/create/", true);
...
//saveAs = projectManager.saveProject
